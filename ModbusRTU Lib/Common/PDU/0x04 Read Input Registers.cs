...
{
    public class ReadInputRegistersPDU : IProtocolDataUnit
    {
        static int ByteCount(int Quantity)
        {
            return (Quantity * 2);
        }
        ...
        public ReadInputRegistersPDU(int StartingAddress, int Quantity)
        {
            ...
            if ((Quantity < 1) || (Quantity > 125))
                throw new ArgumentOutOfRangeException("ReadInputRegisters: Quantity", "Quantity must be in range 1 - 125");
        ...
        public void Handle(IModbusServer Server)
        {
            try
            {
                Value = Server.ReadInputRegisters(StartingAddress, Quantity);
            }
