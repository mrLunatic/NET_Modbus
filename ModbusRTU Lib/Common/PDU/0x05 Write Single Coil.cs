...
{
    public class WriteSingleCoilPDU : IProtocolDataUnit
    {
        static int ConvertToInt(bool Value)
        {
            if (Value)
                return 0xFF00;
            else
        ...
        public WriteSingleCoilPDU(int StartingAddress, bool Value)
        {
        ...
        {
            int tmpValue = ConvertToInt(Value);

            if ((Response.Length == RespSize) && (Response[0] == Func) && ((Response[1] << 8) + Response[2] == StartingAddress) && ((Response[3] << 8) + Response[4] == tmpValue))
                return Status.OK;
        ...
        public Status CheckRequest(byte[] Request)
        {
            ...
            Value = ( ((Request[3] << 8) + Request[4]) == 0xFF00 );
        ...
        public void Handle(IModbusServer Server)
        {
            try
            {
                Server.WriteSingleCoil(StartingAddress, Value);
            }
